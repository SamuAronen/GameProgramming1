using System;
using System.Collections.Generic;
using GameProgramming1.Data;
using GameProgramming1.InputChecks;
using UnityEngine;
using UnityEngine.UI;

public class ControllerSelector : MonoBehaviour
{
    private Dropdown _dropdown;
    public PlayerData.PlayerId Id { get; private set; }
    public InputMethodType Controller { get; private set; }


    public void Init(PlayerData.PlayerId id, InputMethodType defaultInputMethodType)
    {
        _dropdown = GetComponentInChildren<Dropdown>(true);

        _dropdown.ClearOptions();

        List<Dropdown.OptionData> optionDataList = new List<Dropdown.OptionData>();

        foreach (var value in Enum.GetValues(typeof(InputMethodType)))
        {
            if ((InputMethodType) value != InputMethodType.None)
            {
                string controllerName = InputManager.GetControllerName((InputMethodType) value);

                optionDataList.Add(new Dropdown.OptionData(controllerName));
            }
        }

        _dropdown.AddOptions(optionDataList);
        _dropdown.onValueChanged.AddListener(OnValueChanged);


        Id = id;
        Controller = defaultInputMethodType;
        int defaultIndex = GetItemIndex(InputManager.GetControllerName(defaultInputMethodType));

        if (defaultIndex > 0)
        {
            _dropdown.value = defaultIndex;
        }
    }

    private void OnValueChanged(int index)
    {
        Controller = InputManager.GetControllerTypeByName(_dropdown.options[index].text);
    }

    private int GetItemIndex(string controllerName)
    {
        int result = -1;

        for (int i = 0; i < _dropdown.options.Count; i++)
        {
            Dropdown.OptionData item = _dropdown.options[i];
            if (item.text == controllerName)
            {
                result = i;
                break;
            }
        }

        return result;
    }
}